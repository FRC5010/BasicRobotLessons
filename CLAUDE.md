...
- **WPILib Units** (`edu.wpi.first.units`) get peppered in from [Lesson 10](docs/lessons/10-kinematics.md) on — *after* the by-hand math has been taught, never before (Lessons 1–9 keep bare `double`s with unit-suffixed names like `kMaxSpeedMps`, which is the teaching version). Scope is **constants + signatures**: named physical constants (`Distance kWheelDiameter = Inches.of(4)`, `LinearVelocity kMaxSpeed = MetersPerSecond.of(...)`, chassis `Distance`s) and human-authored command parameters (`driveDistance(Distance distance)`, `turnToHeading(Angle target)`). At the boundary where WPILib APIs still take doubles (`ChassisSpeeds`, kinematics, `motor.set`/`setControl`, the per-tick loops) convert with `.in(Meters)` / `.in(Degrees)` / `.in(MetersPerSecond)` — internal math stays in bare doubles so the arithmetic the student just learned stays readable. Dimensionless values (gear ratios, `kP`/`kV` gains) stay `double`. **Unpack only when necessary, and convert once:** when a unit value is needed more than once in a scope (or inside a per-tick lambda), pull it into a local `double` once and reuse it — e.g. `double maxMps = kMaxSpeed.in(MetersPerSecond);` hoisted before the joystick lambdas so they close over the double — rather than calling `.in(...)` repeatedly in a hot path. Migrate the unitless constants gradually: a `double` from an earlier lesson becomes typed only when a lesson has reason to touch it, not in a sweep. Imports: `import static edu.wpi.first.units.Units.*;` for the unit singletons, plus the measure types from `edu.wpi.first.units.measure` (`Distance`, `Angle`, `LinearVelocity`, …). Introduce the library with a teaching beat that builds on the unit-in-the-name habit ("you've been writing `Mps` in the name; the type can carry the unit for you").
