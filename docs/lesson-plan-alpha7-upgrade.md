...
| 2026-09-18 (automated, `NerdSwerveYAGSL2026/tools/check-alpha7-readiness.sh`, cross-checked manually against `frcmaven.wpi.edu`) | `v2027.0.0-alpha-7` (unchanged, no alpha-8) | `2027_alpha7` bucket unchanged (`AdvantageKit-27.0.0-alpha-5.json`, `ChoreoLib-2027.0.0-alpha-3.json`, `REVLib-2027.0.0-alpha-7.json`) — still no Phoenix 6 or `photonlib` entry there | **CTRE's own maven now serves `26.70.0-alpha-2`, and its jar is CLEAN against alpha-7 on the class check** (the check script reads CTRE's own maven metadata, not the marketplace bucket — real signal, but not the same thing as a pin). **The marketplace's Phoenix 6 pin has not moved**: confirmed directly against `frcmaven.wpi.edu/.../vendordep-marketplace/2027_alpha5/Phoenix6-26.50.0-alpha-1.json` — still `"wpilibYear": "2027_alpha5"`, the exact field this doc's own [Verified directly](#verified-directly-how-far-the-lessons-actually-get-today) test showed GradleRIO hard-gates on before it will configure at all. So Track A's *code* is no longer the obstacle, but its *pin* still is | `v2027.0.0-alpha-2` (unchanged) — still 7 classes missing vs. alpha-7: `driverstation/Alert`, `math/util/Pair`, `smartdashboard/SmartDashboard`, `util/sendable/Sendable`, `vision/apriltag/AprilTag`, `vision/apriltag/AprilTagFieldLayout`, `vision/apriltag/AprilTagFields` | **Not yet GO** — Phoenix 6's latest jar is class-clean against alpha-7 for the first time, real progress, but no marketplace-pinned release with a matching `wpilibYear` exists yet, so GradleRIO's hard gate (see above) still refuses to configure the build. Watching for CTRE to re-pin `26.70.0-alpha-2` (or later) into the marketplace under an `alpha6`/`alpha7` bucket — that's the actual Phase 1a trigger | **Blocked** — unchanged, still needs PhotonVision; also inherits Track A's pin wait |
| 2026-09-18, later same day (automated, `NerdSwerveYAGSL2026/tools/check-alpha7-readiness.sh`; pin confirmed directly by fetching the JSON) | `v2027.0.0-alpha-7` (unchanged, no alpha-8) | **`2027_alpha7` bucket gained `Phoenix6-26.70.0-alpha-2.json`** — fetched `raw.githubusercontent.com/wpilibsuite/vendor-json-repo/main/2027_alpha7/Phoenix6-26.70.0-alpha-2.json` directly and confirmed `"wpilibYear": "2027_alpha7"`. This is exactly the Phase 1a trigger the previous row was watching for: a marketplace-pinned Phoenix 6 release with a matching `wpilibYear`, not just a class-clean jar on CTRE's own maven. Still no `photonlib` entry in the bucket | `26.70.0-alpha-2`, now marketplace-pinned under `2027_alpha7` with a matching `wpilibYear`, and CLEAN against alpha-7 on the class check (unchanged from the morning's finding) | `v2027.0.0-alpha-2` (unchanged) — still 7 classes missing vs. alpha-7: `driverstation/Alert`, `math/util/Pair`, `smartdashboard/SmartDashboard`, `util/sendable/Sendable`, `vision/apriltag/AprilTag`, `vision/apriltag/AprilTagFieldLayout`, `vision/apriltag/AprilTagFields` | **GO** — both of Track A's gates are now clear: the jar is class-clean and it's marketplace-pinned with a matching `wpilibYear`, so GradleRIO's hard gate no longer applies. **Phase 1a can start** — the two same-repo prerequisites (`CommandsV3.json` refresh, `build.gradle` deploy-artifact wiring) are still open unknowns independent of this, per [Phase 1a](#phased-plan) | **Blocked** — unchanged, still needs PhotonVision (7 classes missing, no marketplace entry) |
