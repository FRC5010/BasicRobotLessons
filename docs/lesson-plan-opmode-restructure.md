...
| 25 | Path events | **Skipped for now — unblocked 2026-09-22, not yet written** | 2026-08-15: same root cause as Lesson 17 (R2) — this lesson's entire content is BLine event-marker/`overrideRotation` machinery layered on `FollowPath`, which is already confirmed structurally incompatible with this alpha's Commands V3. **User decision (2026-08-15): skip, no interim, same call as 17** — proceed to Lesson 26, which doesn't need BLine's generated-path machinery for its own teaching content. Stays a gap alongside 17 and 22 until BLine ships `org.wpilib.*`-compatible support. **2026-09-22: unblocked alongside 17 — see R2's update.** `registerEventTrigger`/`overrideRotation` both confirmed present in `v2027.0.0-beta.1` via `javap` (the latter gained a `RotationOverrideBehavior` overload not in the old API). Not yet written; inserting it between 24 and 26 will need the `Next:`/back-reference fixes CLAUDE.md calls for |
...
- **R2 — split three ways as of 2026-08-13: PhotonVision de-risked, maple-sim
  ...
  **Update 2026-09-22: BLine's side of this is resolved, verified by a real
  ...
  already burned this course once, on `EventTrigger`). **This unblocks
  Lesson 17 and Lesson 25 for Track B's remaining gap** — Lesson 22 stays
  blocked separately on maple-sim, untouched by this finding. Scope and
  timing of actually writing 17/25 is a fresh open decision for the team,
...
- [x] Lesson 17 (BLine autos) investigated and skipped, 2026-08-13 — no
      ...
      support.
- [x] Lesson 17 unblocked, 2026-09-22 — `EdanLiahovetsky/BLine-Lib`
      ...
      Lesson 17 is still not written — this only clears the blocker.
...
- [x] Lesson 25 unblocked alongside Lesson 17, 2026-09-22 — same
      ...
      in the old `v0.9.1` API this lesson's design assumed). Still not
      written; note the `Next:`/back-reference link fixes CLAUDE.md
      requires when inserting a lesson between two that already exist
      (24 and 26).
