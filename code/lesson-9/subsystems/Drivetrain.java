...
public class Drivetrain extends SubsystemBase {
    ...
    public Command driveDistance(double meters) {
        return runOnce(() -> m_modules[0].resetDrivePosition()) // 1. zero one wheel's odometer
                .andThen(run(() -> {
                    for (SwerveModule module : m_modules) {
                        module.setDesiredState(0.0, 0.4); // 2. point forward, 40% throttle
                    }
                    m_lastCommandedOmega = 0.0;
                }))
                // 3. ...until that wheel has covered the distance
                .until(() -> Math.abs(m_modules[0].getDistanceMeters()) >= Math.abs(meters))
                .finallyDo(() -> {
                    for (SwerveModule module : m_modules) {
                        module.setDesiredState(0.0, 0.0); // 4. stop
                    }
                });
    }
